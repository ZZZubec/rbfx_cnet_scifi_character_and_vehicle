...
namespace CarSandboxGame
{
    public class GameState : ApplicationState
    {
        ...
        private SharedPtr<Vehicle2> _vehicle;

        private Vehicle2 Vehicle
        {
            get
            {
                if (_vehicle == null || _vehicle.Ptr.IsExpired)
                    return null;
                return _vehicle;
            }
            set
        ...
        {
            var vehicleNode = _scene.Ptr.CreateChild("Vehicle");
            vehicleNode.Position = new Vector3(1.68f, 1.11f, 1.46f);
            // Create the vehicle logic component
            Vehicle = vehicleNode.CreateComponent<Vehicle2>();
            // Create the rendering and physics components
            Vehicle.Init();
            
            Vehicle.AttachCamera = vehicleNode.CreateChild("attachCamera");
            Vehicle.AttachCamera.Position = new Vector3(0, 1.5f, -2f);

            var player = _character.GetComponent<Player>();
